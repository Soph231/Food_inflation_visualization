...
country_year_mean = pd.read_csv('country_year_mean.csv')
...
def update_stacked_barplot():  # Add parameters as needed
    ...
    fig = px.bar(df, 
                 ...
                 title='Continent-wise Distribution of Inflation Types',
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Button('Countries- Line Plot', id='btn-line', n_clicks=0, className='inactive'),
    ...
    html.Div(style={'display': 'flex', 'justify-content': 'space-between', 'align-items': 'center', 'margin-top': '10px'}, children=[
        # Year Dropdown with its title underneath
        html.Div([
            html.H5("Years:", style={'text-align': 'center', 'font-size': '12px','margin-top': '10px'}),
            dcc.Dropdown(
                id='year-dropdown',
                options=[{'label': str(year), 'value': year} for year in range(2001, 2025)],
                placeholder="Select the year", 
                value=2024,  # Automatically select 2024 as the default year
                style={'width': '100%', 'margin-top': '10px'}
            ),
        ], style={'width': '30%'}),  # Set width for each dropdown and title container
        
        # Category Dropdown with its title underneath
        html.Div([
            html.H5("Inflation Categories:", style={'text-align': 'center', 'font-size': '12px', 'margin-top': '10px'}),
        ...
        # Country dropdown linked to the scatter plot
        html.Div([
            html.H5("Countries List:", style={'text-align': 'center', 'font-size': '12px', 'margin-top': '10px'}),
            dcc.Dropdown(id='country-dropdown', 
                placeholder="Select a country", style={'width': '100%', 'align-items': 'center','margin-top': '10px'}),
        ], style={'width': '30%'})
...
# Update country dropdown with unique countries (all countries if no category selected)
@app.callback(
    Output('country-dropdown', 'options'),
    Input('category-dropdown', 'value')
)
def update_country_dropdown(selected_category):
    # If no category is selected, show all countries
    if selected_category is None:
        all_countries = country_year_mean['Area'].unique()  # Get all countries
        return [{'label': country, 'value': country} for country in sorted(all_countries)]
    
    # Otherwise, filter countries by selected inflation category
    filtered_data = country_year_mean[country_year_mean['Inflation_Category'] == selected_category]
    countries = filtered_data['Area'].unique()
...
def toggle_dropdown_visibility(map_clicks, line_clicks, bar_clicks):
    ...
    if button_id == 'btn-map':
        ...
        # Show category and country dropdowns for the line plot, hide year dropdown
...
def set_button_style(map_clicks, line_clicks, bar_clicks):
    ctx = dash.callback_context
...
def update_active_plot(map_clicks, line_clicks, bar_clicks):
    ctx = dash.callback_context
    if not ctx.triggered:
    ...
    if triggered_button == 'btn-map':
        return 'map'
    elif triggered_button == 'btn-line':
        return 'line'
...
def update_plot_area(year, selected_category, selected_country, active_plot):
    if year is None:
        year = 2024
    ...
    elif active_plot == 'line':
        return dcc.Graph(figure=update_country_graph(selected_country, selected_category))
...
def update_map(year, selected_category):
    ...
    try:
        geojson_file = f'Inflation_data/inflation_{year}.geojson'
...
#helper function to create country-wise scatter plot    
# Cache filtered data for faster access in the plot function
@cache.memoize(timeout=300)
def filter_data(selected_country, selected_category):
    # Filter by category if selected
    if selected_category:
        filtered_data = country_year_mean[country_year_mean['Inflation_Category'] == selected_category]
    else:
        filtered_data = country_year_mean
    
    # Filter by country if selected, otherwise use default
    if selected_country:
        country_data = filtered_data[filtered_data['Area'] == selected_country]
    return filtered_data, country_data

# Optimized country-wise scatter plot function
@cache.memoize(timeout=300)
def update_country_graph(selected_country, selected_category):
    # Get the filtered data
    filtered_data, country_data = filter_data(selected_country, selected_category)

    # Default plot if nothing is selected (Afghanistan)
    if not selected_country and not selected_category:
        title = 'Inflation Rate in Afghanistan Over the Years'
        fig = px.scatter(
            country_data,  # Default to Afghanistan's data
            x='Year',
            y='Value',
            color='Area',
            hover_data=['Area'],
    ...
    elif selected_country and not selected_category:
        title = f'Inflation Rates for {selected_country} Over the Years'
        fig = px.scatter(
            country_data,
            x='Year',
            y='Value',
    ...
    # If category is selected (with or without country)
    else:
        title = f'Inflation Rates for {selected_category}'
        fig = px.scatter(
            filtered_data,
            x='Year',
            y='Value',
        ...
        # If a specific country is selected within the category, highlight it
        if selected_country:
            fig.add_scatter(
                x=country_data['Year'],
                y=country_data['Value'],
                mode='markers',
                marker=dict(size=12, symbol='star', color='red', line=dict(width=2, color='DarkSlateGrey')),
                name=f'{selected_country} (Highlighted)',
                hoverinfo='x+y+text',
                text=selected_country
            )

    # Update layout with common elements
    fig.update_layout(
        legend=dict(
            orientation="h",
            yanchor="top",
